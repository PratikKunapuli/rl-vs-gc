...
class NMPC:
    def __init__(self, mass, inertia, x0, goal_pos):
        self.sim_time = 50 # total timesteps for simulation
        self.Ts = 0.02 # sampling time in seconds
        self.T_horizon = 20 # prediction horizon in timesteps 
        self.N = int(self.T_horizon/self.Ts) # total number of timesteps in the horizon
        self.mass = mass
        self.inertia = inertia
        self.gravity_vec = np.array([0, 0, 9.81])
        self.x0 = x0 # Will be (n_envs, n_states)
        # self.u0 = None
    ...
    def define_model(self):
        ...
        # Define cost
        model, cost_expression = self.get_cost_expression(model)
...
if __name__ == "__main__":
    ...
    x0 = np.vstack((init_x, init_v, init_R, init_omega))
    ...
    time_axis = np.arange(n_query_steps+1) * nmpc.Ts
    plt.figure()
    ...
    plt.figure()
    time_axis = np.arange(n_query_steps) * nmpc.Ts
    plt.subplot(211)
    plt.plot(time_axis, action_list[:,0], label='thrust')
    plt.subplot(212)
